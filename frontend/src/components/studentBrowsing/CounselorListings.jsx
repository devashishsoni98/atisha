            ...
            <div className="p-6">
                <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-grow">
                        ...
                        <p className="text-sm text-gray-600 mb-2">{counselor.counselor_professional?.counselor_speciality}</p>
                        <div className="flex items-center mb-2">
                            <Star className="w-4 h-4 text-yellow-400 mr-1" />
                            <span className="text-sm font-medium">{counselor.similarityScore?.toFixed(2)}</span>
                            <span className="text-sm text-gray-600 ml-1">similarity score</span>
                        </div>
                        <div className="flex flex-wrap gap-2 mb-2">
                            {counselor.counselor_professional?.career_specialization.slice(0, 3).map((exp, index) => (
                                <span key={index} className="text-xs px-2 py-1 bg-gray-200 rounded-full">{exp}</span>
                            ))}
                        </div>
...
const CounselorDetails =({ counselor })=> {
    ...
    const handleBookingConfirm = async () => {
        if (selectedSlot) {
            setLoading(true)
            try {
                const result = await bookSlot(counselor.id, selectedSlot.id)
    ...
    return (
        <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <p className="text-gray-600 mb-6">{counselor.counselor_professional.counselor_speciality}</p>
                ...
                {selectedCounselor ? (
                    ...
                    <div className="flex items-center justify-center h-full bg-gray-200 rounded-lg p-6">
                        <p className="text-gray-600 text-center">
                            Select a counselor to view available time slots and book an appointment.
                        </p>
                    </div>
                )}
            </section>
