...
//       navigate('/lens-details', { state: { careerData: parsedData } });
//     } catch (error) {
...
//     try {
//       // If trendData is already an array, return it
//       if (Array.isArray(trendData)) {
//         return trendData;
//       }

//       // If trendData is a string, try to parse it
//       if (typeof trendData === 'string') {
//         // Remove any leading/trailing whitespace and replace single quotes with double quotes
...
//       // If trendData is an object, convert it to an array
...
const CareerLens = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const validCareers = ["Software Engineer", "Doctor", "Scientist"];
      const selectedCareer = validCareers.find(
        (c) => c.toLowerCase() === career.toLowerCase()
      );

      if (selectedCareer) {
        navigate('/lens-details', { state: selectedCareer });
      } else {
        throw new Error('Invalid career selected');
      }
    } catch (error) {
